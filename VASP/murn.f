...
c      data convyy, kb /13.605826, 3.16666225714e-6/
      data convyy, kb /1.d0, 3.16666225714e-6/
...
c      read(in,*) tzero, gamma, thetad, atvol
...
c number of data points a, e(a):
 ...
 1030 format('# number of data points: n =',i5)
      ...
      write(iout,1120)
      write(iout,1040)
 1040 format('#    i',5x,'alatt',9x,'volume',12x,'  E ')
      do 10 i=1,nnn
        read(in,*)alatt,eninp
        alatt=alatt
...
c murnaghan least-squares fit:
 ...
 1190 format('#iter, alatt0, vol0 (ula**3),' 
     1  ,'  b0 (mbar),  b0prime,  e0,  sq sum, ierr')
      ...
      do 70 i = 1, 75
        ...
        write(iout,'("# ",i3,f9.4,3f12.5,f14.7,f12.5,i2)')
     &       20*i,(x(4)/convav)**(1.d0/3.d0),x(4),x(2),x(3),
     &       (x(1)-shift)/conv_inp,fff,ierr
